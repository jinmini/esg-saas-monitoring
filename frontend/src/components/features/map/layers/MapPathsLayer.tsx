/**
 * MapPathsLayer
 * SVG 지도 배경 레이어 (국가 경계선)
 * 
 * 전략:
 * - 14개 타겟 유럽 국가: 개별 path 관리 (Interactive)
 * - 나머지 국가: 그룹화 (Non-interactive, 배경)
 * 
 * 성능 최적화:
 * - 타겟 국가만 이벤트 리스너 부착
 * - 나머지는 pointer-events: none
 */
...
interface MapPathsLayerProps {
  opacity?: number; // 배경 지도 투명도 (기본: 0.3)
  showOtherCountries?: boolean; // 타겟 외 국가 표시 여부
}

/**
 * 타겟 국가 Path 데이터
 * world.svg에서 추출한 14개 유럽 국가
 */
const TARGET_COUNTRY_PATHS: Partial<Record<CountryCode, string>> = {
