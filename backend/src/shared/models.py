...
class TimestampMixin:
    """타임스탬프 공통 mixin"""
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False, index=True)  # ✅ index 추가
...
class Document(Base, TimestampMixin):
    ...
    versions = relationship("DocumentVersion", back_populates="document", cascade="all, delete-orphan", order_by="desc(DocumentVersion.created_at)")  # ✅ 버전 관계 추가
...
class Section(Base, TimestampMixin):
    ...
    blocks = Column(JSONB, nullable=False, default=[])  # ✅ JSON → JSONB
    gri_reference = Column(JSONB, nullable=True)  # ✅ JSON → JSONB
    # metadata는 SQLAlchemy 예약어이므로 section_metadata로 저장, Pydantic에서 alias 처리
    section_metadata = Column("metadata", JSONB, nullable=True)  # ✅ JSON → JSONB
